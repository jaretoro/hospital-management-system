          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-6 flex items-center justify-between">
            <div className="flex flex-col gap-4">
              ...
              <button
                ...
                className="bg-orange-500 text-white px-6 py-2.5 rounded-xl text-sm font-semibold hover:bg-orange-600 transition w-fit"
              >
                Start Consultation
              </button>
            </div>
            <img src="/stethoscope.png" alt="stethoscope" className="w-32 h-32 object-contain" />
          </div>

          {/* Report Analysis Chart */}
          <div className="bg-white rounded-2xl border border-slate-100 p-6">
            ...
            <ResponsiveContainer width="100%" height={180}>
              <AreaChart data={chartData} margin={{ top: 0, right: 0, left: -30, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="value" stroke="#FF7221" strokeWidth={2.5} fill="url(#orangeGrad)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
            ...
            <div className="flex flex-col gap-4">
              {patientQueue.map((patient, index) => (
                <div key={index} className="flex items-center gap-3">
                  ...
                  <button className="bg-orange-500 text-white px-4 py-1 rounded-lg text-xs hover:bg-orange-600 transition">
                    Start
                  </button>
                </div>
              ))}
            ...
            <div className="flex flex-col gap-4">
              {recentConsultations.map((recent, index) => (
                <div key={index} className="flex items-center gap-3">
                  ...
                  <span className="text-[10px] bg-slate-50 text-slate-400 px-2 py-1 rounded-full">
                    {recent.time}
                  </span>
                  <button className="bg-orange-500 text-white px-4 py-1 rounded-lg text-xs hover:bg-orange-600 transition">
                    Start
